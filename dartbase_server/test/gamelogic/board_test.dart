...
void main() {
  group('board instance tests', () {
    ...
    test('play normal card', () {
      ...
      expect(
          board.playCardToStation(
              BoardLoc.origin, Card.pow, CardDirection.up, 1),
          isTrue);
    ...
    test('play normal card in non-origin', () {
      ...
      expect(
          board.playCardToStation(
              const BoardLoc(5, 4), Card.pow, CardDirection.up, 1),
          isTrue);
      expect(board.count, 1);
      expect(
          board.playCardToStation(
              const BoardLoc(5, 3), Card.pow, CardDirection.up, 1),
          isFalse);
      expect(board.count, 1);
      expect(
          board.playCardToStation(
              const BoardLoc(1, 0), Card.pow, CardDirection.up, 1),
          isTrue);
  ...
  group('is legal move', () {
    test('first card is always legal move', () {
      ...
      for (CardDirection dir in CardDirection.values) {
        expect(
            Card.values
                .every((card) => board.isLegalMove(BoardLoc.origin, card, dir)),
            isTrue);
      }
    });
    ...
    test('legal pow card placement next to cap', () {
      ...
      expect(
          board.playCardToStation(
              BoardLoc.origin, Card.rec, CardDirection.up, 1),
          isTrue);
      expect(board.count, 1);
  ...
  group('areConnected tests', () {
    test('three neighbors with two exits', () {
      ...
      expect(
          board.areConnected(BoardLoc.origin,
              BoardLoc.origin.neighborLoc(CardDirection.right)),
          isTrue);
      expect(
          board.areConnected(
              BoardLoc.origin, BoardLoc.origin.neighborLoc(CardDirection.down)),
          isTrue);
      expect(
          board.areConnected(new BoardLoc(7, 8),
              BoardLoc.origin.neighborLoc(CardDirection.down)),
          isFalse);
      expect(
          board.areConnected(
  ...
  group('sabotage legal move tests', () {
    test('sabotage is a legal move on empty board', () {
      Board board = new Board();
      expect(board.isLegalMove(BoardLoc.origin, Card.sab, CardDirection.up),
          isTrue);
      expect(
          board.isLegalMove(const BoardLoc(1, 1), Card.sab, CardDirection.up),
          isTrue);
    });
    test('sabotage legal board loc checks', () {
      ...
      expect(
          ...
          isFalse);
      expect(
          board.isLegalMove(const BoardLoc(1, 0), Card.sab, CardDirection.up),
          isTrue);
      expect(
          board.isLegalMove(const BoardLoc(0, 1), Card.sab, CardDirection.up),
          isFalse);
    });
  });
  group('sabotage placement tests', () {
    ...
    test('sabotage three card board', () {
      ...
      expect(
          board.playCardToStation(BoardLoc.origin.neighborLoc(CardDirection.up),
              Card.sab, CardDirection.up, 1),
          isFalse);
      expect(
          board.playCardToStation(
              BoardLoc.origin, Card.sab, CardDirection.up, 1),
          isTrue);
      expect(board.fringe.length, 6);
      expect(board.count, 2);
    });
  ...
  group('board location tests', () {
    test('neighbor location', () {
      expect(BoardLoc.origin.neighborLoc(CardDirection.right),
          const BoardLoc(1, 0));
      expect(BoardLoc.origin.neighborLoc(CardDirection.left),
          const BoardLoc(-1, 0));
      expect(
          BoardLoc.origin.neighborLoc(CardDirection.up), const BoardLoc(0, 1));
      expect(BoardLoc.origin.neighborLoc(CardDirection.down),
          const BoardLoc(0, -1));
    });
    test('from constructor', () {
      expect(new BoardLoc.from(BoardLoc.origin), const BoardLoc(0, 0));
      expect(new BoardLoc.from(BoardLoc.origin.neighborLoc(CardDirection.left)),
          const BoardLoc(-1, 0));
    });
  });
  ...
  group('payment path group', () {
    test('Get paypaths initial implementation', () {
      ...
      expect(payPaths[0].length, 2);
      expect(payPaths[0], contains(BoardLoc.origin));
      expect(payPaths[0], 
